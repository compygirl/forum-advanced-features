...
func (postObj *PostServiceImpl) Filter(field string, userID int) ([]*models.Post, error) {
	...
	if field == "CreatedPosts" {
		...
		// fmt.Println("FILTERING BY CATEGORY")
		field = strings.ToLower(field)
		// fmt.Println("CATEGORY: ", field)
		posts, err = postObj.repo.GetPostsByCategory(field)
		if err != nil {
			fmt.Println("FILTER: SERVICE LEVEL")
			return nil, err
		}
	}
	return posts, nil
...
func (postObj *PostServiceImpl) ChangeReportStatusOfPostbyPostID(postID int, reportStatusValue int) error {
	err := postObj.repo.ChangeReportStatusOfPostbyPostID(postID, reportStatusValue)
	if err != nil {
		return err
	}
	return nil
...
func (postObj *PostServiceImpl) AddPostReportCategory(postID int, reportCategory string) error {
	err := postObj.repo.AddPostReportCategory(postID, reportCategory)
	if err != nil {
		return err
	}
	return nil
...
func (postObj *PostServiceImpl) GetAllCategories() ([]*models.Category, error) {
	categories, err := postObj.repo.GetAllCategories()
	if err != nil {
		return nil, err
	}
	return categories, nil
...
func (postObj *PostServiceImpl) DeletePostCategory(CategoryID int) error {
	err := postObj.repo.DeletePostCategoryByCategoryID(CategoryID)
	if err != nil {
		return err
	}
	return nil
...
func (postObj *PostServiceImpl) CreateCategory(CategoryName string) (int, int, error) {
	id, err := postObj.repo.CreateCategory(CategoryName)
	if err != nil {
		return http.StatusBadRequest, -1, err
	}
	return http.StatusOK, int(id), err
}

func (postObj *PostServiceImpl) UpdatePostContentByPostID(postID int, content string) error {
	err := postObj.repo.UpdatePostContentByPostID(postID, content)
	if err != nil {
		return err
	}
	return nil
...
func (postObj *PostServiceImpl) GetPostsByUserId(userId int) ([]*models.Post, error) {
	posts, err := postObj.repo.GetPostsByUserId(userId)
	if err != nil {
		return nil, err
	}
...
func (postObj *PostServiceImpl) GetMyReactedPosts(userID int) (map[int]int, error) {
	mapa, err := postObj.repo.GetMyReactedPosts(userID)
	if err != nil {
		return nil, err
	}
	return mapa, nil
...
func (postObj *PostServiceImpl) GetAllMyPostsLikedByOtherUsers(userID int) ([]*models.PostVotes, error) {
	postVotes, err := postObj.repo.GetAllMyPostsLikedByOtherUsers(userID)
	if err != nil {
		return nil, err
	}
...
func (postObj *PostServiceImpl) GetAllMyPostsCommentedByOtherUsers(userID int) ([]*models.PostVotes, error) {
	postVotes, err := postObj.repo.GetAllMyPostsCommentedByOtherUsers(userID)
	if err != nil {
		return nil, err
	}
	return postVotes, nil
